...
export default {
    ...
    methods: {
        async getPedidos() {
            // Obter pedidos
            const request = await  fetch("https://json-backend.vercel.app/burgers")
            this.burgers = await request.json()
        },
        async getStatus(){
            // Obter status
            const requestStatus = await fetch("https://json-backend.vercel.app/status")
            this.status = await requestStatus.json()
        },
        async deleteBurger(id){
            const request = await fetch(`https://json-backend.vercel.app/burgers/${id}`, {
                method: "DELETE"
            })
            this.getPedidos()
